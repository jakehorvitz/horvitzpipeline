#!/usr/bin/env bash
#
# bones-guard.sh v2 (Horvitz 2.0) — PreToolUse hook: makes bones gates bind at the HARNESS level.
#
...
#
#   GLOBAL (no tree needed):
#     T1. .bones/ audit-trail tamper protection — only read-only commands may
#         mention .bones paths; writes/deletes/redirects are blocked wherever the
#         agent stands (a forged gate file is worse than a skipped gate). `bones reset`
#         and `bones init -f` are Jake's call from his own terminal.
#   WHICH TREE: every tree the command can touch is evaluated — the session cwd's run,
#     the `cd`/`pushd` target's run, and any `git -C <dir>` / `make -C <dir>` /
#     `--cwd <dir>` target's run. Leaving the tree with `cd ..` does not unguard it.
...
#     T2 (2.0). the INSTALLED orchestrator (~/.claude/skills/horvitz-pipeline,
#         ~/.claude/skills/ship-pipeline) is read-only for an agent inside an active run
#         until 8a is authorized for the current HEAD (or the run is past 8).
#   STAGE 8 (2.0): a VALID owner token in gates/8-promote.authorized — signed by the
#     pinned owner key (.bones/owner.pub), bound to the CURRENT git HEAD and a CLEAN
#     working tree — lifts the pre-promote rules below. Anything else at stage 8 is
#     blocked like 1..7.
...
INSTALLED_RE='\.claude/skills/(horvitz-pipeline|ship-pipeline)(/|$)'

# ---- helpers ---------------------------------------------------------------
find_tree() { # find_tree <dir> -> prints the nearest .bones dir walking up, or nothing
  local d="$1"
  [ -d "$d" ] || return 1
  d="$(cd "$d" 2>/dev/null && pwd)" || return 1
  while [ "$d" != "/" ] && [ -n "$d" ]; do
    if [ -d "$d/.bones" ]; then printf '%s\n' "$d/.bones"; return 0; fi
    d="$(dirname "$d")"
  done
  return 1
}
resolve_dir() { # resolve_dir <path> <base> -> absolute path (unexpanded ~ handled)
  local p="$1" base="$2"
  p="${p%\"}"; p="${p#\"}"; p="${p%\'}"; p="${p#\'}"
  case "$p" in
    "~"|"~/") p="$HOME" ;;
...
is_write_shape() { # rc 0 when the Bash command can write/delete/move
  printf '%s' "$1" | grep -qE '(>|>>|[[:space:]]tee[[:space:]]|^tee[[:space:]]|sed[[:space:]]+.*-i|perl[[:space:]]+.*-i|(^|[;&|][[:space:]]*)(rm|mv|cp|chmod|chown|truncate|install|ln|mkdir|touch|dd)[[:space:]])'
}

# ---- GLOBAL T1: .bones/ is read-only for agents, wherever they stand ----------------
if [ "$tool" = "Bash" ] && [ -n "$cmd" ]; then
  ...
  if printf '%s' "$cmd" | grep -qE '\.bones(/|[[:space:]]|"|'\''|$)'; then
    ...
    if ! printf '%s' "$cmd" | grep -qE '^[[:space:]]*(cat|ls|head|tail|grep|diff|find|wc|stat|shasum|sha256sum|file)[[:space:]][^;&|<>]*$'; then
      block "tamper" "Only bones.sh may write the .bones/ audit trail — direct edits forge gate records (this rule applies from any directory)."
    fi
  fi
...
if [ "$tool" != "Bash" ] && [ -n "$file_path" ]; then
  case "$file_path" in
    */.bones/*|.bones/*)
      block "tamper" "Only bones.sh may write the .bones/ audit trail — file-edit tools cannot modify gate records, state, pinned acceptance, or the owner key pin." ;;
  esac
fi

# ---- which trees does this call touch? ------------------------------------------------
trees=""
add_tree() { local t; t="$(find_tree "$1" 2>/dev/null)" || return 0; [ -n "$t" ] || return 0; case " $trees " in *" $t "*) ;; *) trees="$trees $t" ;; esac; }
add_tree "$session_cwd"
if [ "$tool" = "Bash" ]; then
  cd_target="$(printf '%s' "$cmd" | sed -nE "s/^[[:space:]]*(cd|pushd)[[:space:]]+[\"']?([^\"';&| ]+)[\"']?.*/\2/p")"
  [ -n "$cd_target" ] && add_tree "$(resolve_dir "$cd_target" "$session_cwd")"
  # commands that act on another directory explicitly
  while IFS= read -r d; do [ -n "$d" ] && add_tree "$(resolve_dir "$d" "$session_cwd")"; done < <(printf '%s' "$cmd" | grep -oE '(git[[:space:]]+-C|make[[:space:]]+-C|--cwd)[[:space:]]*=?[[:space:]]*("[^"]+"|'\''[^'\'']+'\''|[^[:space:];&|]+)' | sed -E 's/^(git[[:space:]]+-C|make[[:space:]]+-C|--cwd)[[:space:]]*=?[[:space:]]*//')
else
  [ -n "$file_path" ] && add_tree "$(dirname "$(resolve_dir "$file_path" "$session_cwd")")"
fi
[ -n "$trees" ] || exit 0
...
guard_tree() {
  local bones="$1"
  [ -f "$bones/state.json" ] || return 0
  stage="?"; name="?"; auth_why=""

  # Seal-on-read (2.0): an unsealed run may still be inspected and healed — allow ONLY a bare
  # `bones.sh status|doctor` with literal paths (no $, backticks, parentheses, redirects, chaining).
  ...
  local schema; schema="$(jq -r '.schema_version // empty' "$bones/state.json" 2>/dev/null)"
  case "$stage" in
  ...
  # ---- 8a authorization (2.0): at stage 8 a VALID owner token bound to the current HEAD and a
  #      CLEAN tree lifts the promote rules. Anything else at stage 8 is blocked like stages 1..7.
  local auth_ok=0
  if [ "$stage" -eq 8 ]; then
    ...
    if [ -f "$bones/gates/8-promote.authorized" ]; then
      ...
      local tok tgt head_now dirty_now payload sig pub tv tn ts th tq tz tt gw
      ...
      if [ -n "$tok" ] && [ -f "$bones/owner.pub" ] && command -v openssl >/dev/null 2>&1; then
        ...
        # F-02: the authorization binds the COMMITTED tree (changes outside .bones/.loop make it unusable)
        dirty_now=""
        if [ "$head_now" != nogit ]; then dirty_now="$(git -C "$tgt" status --porcelain 2>/dev/null | grep -vE '^.. (\.bones|\.loop)(/|$)' | head -1)"; fi
        [ -n "$dirty_now" ] && auth_why="working tree has uncommitted changes — commit or stash; 8a binds the committed tree"
  ...
  # Past the promote gate (stage 9+): promote-shaped commands and skill maintenance are legitimate.
  [ "$stage" -le 8 ] || return 0
  # 8a authorized for the CURRENT head: the same applies.
  [ "$auth_ok" -eq 1 ] && return 0

  # ---- T2 (2.0): the installed orchestrator is read-only for an agent inside an active run ----
  if [ "$tool" != "Bash" ]; then
    if printf '%s' "$file_path" | grep -qE "$INSTALLED_RE"; then
      block "skill-tamper" "The installed orchestrator (~/.claude/skills/horvitz-pipeline, ship-pipeline) is read-only while a run is open — an agent must not edit the gates that gate it. Upgrades land at promote (stage 8a authorized) or from outside any active run."
    fi
    return 0
  fi
  [ -n "$cmd" ] || return 0
  if printf '%s' "$cmd" | grep -qE "$INSTALLED_RE" && is_write_shape "$cmd"; then
    block "skill-tamper" "The installed orchestrator (~/.claude/skills/horvitz-pipeline, ship-pipeline) is read-only while a run is open — an agent must not edit the gates that gate it. Upgrades land at promote (stage 8a authorized) or from outside any active run."
  fi

  # ---- G1: git push allowlist --------------------------------------------------
  if printf '%s' "$cmd" | grep -qE '(^|[;&|({][[:space:]]*)git([[:space:]]+-[Cc][[:space:]]+[^[:space:]]+)?[[:space:]]+push([[:space:]]|$)'; then
    local push_part protected extra
  ...
  local verbs='(deploy|publish|release|promote)'
  ...
  local cli_re='(vercel([[:space:]]+deploy)?[^|;&]*--prod)'
...
for t in $trees; do guard_tree "$t"; done
exit 0
